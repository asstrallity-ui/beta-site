...
if (btnStartUpdate) {
    btnStartUpdate.addEventListener('click', () => {
        btnStartUpdate.innerHTML = '<span class="material-symbols-outlined spin">refresh</span> Скачивание...';
            ...
            <div class="big-panel">
                ...
                <div class="custom-color-picker">
                    <div class="picker-header">
                        <div class="current-color-preview" style="background-color: var(--md-sys-color-primary);"></div>
                        <div class="picker-info">
                            <h3>Акцентный цвет</h3>
                            <p>Выберите цвет интерфейса</p>
                        </div>
                    </div>
                    <div class="picker-controls">
                        <label>Оттенок</label>
                        <input type="range" class="slider-hue" min="0" max="360" value="0" id="hue-slider">
                    </div>
                    <div class="presets-grid">
                        ...
                        <div class="color-preset" style="background:#a0c1ff" onclick="updateColor('#a0c1ff')"></div>
                        <div class="color-preset" style="background:#80cbc4" onclick="updateColor('#80cbc4')"></div>
                        <div class="color-preset" style="background:#fff59d" onclick="updateColor('#fff59d')"></div>
                    </div>
...
window.updateColor = (color) => {
    ...
    const prev = document.querySelector('.current-color-preview');
...
async function checkEnvironment() {
    ...
    try {
        ...
        const verBadge = document.querySelector('.app-version-badge');
        if(verBadge && info.app_version) verBadge.innerText = `LOADER ASTR v${info.app_version}`;
...
navItems.forEach(item => {
    item.addEventListener('click', () => {
        const id = item.id;
        setActiveNav(id);
        if (id === 'nav-catalog') loadMods();
        if (id === 'nav-methods') renderInstallMethods();
        if (id === 'nav-authors') loadAuthors();
        if (id === 'nav-settings') renderSettings();
    });
});
...
function renderMods(mods, installedIds, buyList) {
    ...
    mods.forEach(mod => {
        ...
        if (buyInfo) {
            ...
            if (!window.pywebview) {
                btnText = 'Доступно в приложении'; isDisabled = true;
            } else if (isInst) {
...
function renderInstallMethods() {
    ...
    function upd(m) {
        ...
        cA.classList.remove('active-method'); cS.classList.remove('active-method'); cN.classList.remove('active-method');
        if(m==='auto')cA.classList.add('active-method');
        if(m==='sdls')cS.classList.add('active-method');
        if(m==='no_sdls')cN.classList.add('active-method');
    }
...
async function loadAuthors() {
    ...
    try {
        ...
        contentArea.innerHTML = `<p style="color:#ff5252;">Ошибка авторов.</p>`;
    }
}
...
window.finishInstall = (s, m) => {
    if(s) {
        ...
        setTimeout(() => { closeModal(); loadMods(); }, 2000);
    } else {
...
function openRepairModal() {
    ...
    if (installedMods.length === 0) {
        repairList.innerHTML = '<div class="empty-state" style="height:100px; font-size:13px;"><p>Нет установленных модов для починки.</p></div>';
    } else {
        installedMods.forEach(mod => {
            ...
            item.innerHTML = `
                <span>${mod.name}</span>
                <button class="repair-action-btn" onclick="restoreMod('${mod.id}', '${mod.name}')" title="Починить/Удалить">
